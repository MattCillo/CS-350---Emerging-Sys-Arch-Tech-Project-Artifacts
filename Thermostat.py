...
class TemperatureMachine(StateMachine):
    ...
    def manageMyDisplay(self):
        ...
        while not self.endDisplay:
            ...
            lcd_line_1 = current_time.strftime("%b %d %H:%M:%S")[:16] + "\n"

            if(altCounter < 6):
                lcd_line_2 = f"T:{self.getFahrenheit():4.1f}F".ljust(16)
                altCounter = altCounter + 1
            else:
                lcd_line_2 = f"{self.current_state.id.upper()} SP:{self.setPoint}F".ljust(16)
                altCounter = altCounter + 1
                if(altCounter >= 11):
                    self.updateLights()
                    altCounter = 1

            screen.updateScreen(lcd_line_1 + lcd_line_2[:16])
...
while repeat:
    try:
        ...
        blueLight.off()
        sleep(1)
        if hasattr(tsm, 'myThread'):
            tsm.myThread.join(timeout=2)
        ser.close()